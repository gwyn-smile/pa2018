...
static inline void rtl_add_i(rtlreg_t* dest, const rtlreg_t* src, const rtlreg_t* imm8) {
	*dest = *src + *imm8;
}
...
static inline void rtl_xor_i(rtlreg_t* dest, const rtlreg_t* src, const rtlreg_t* imm8) {
	*dest = *src ^ *imm8;
}
...
static inline void rtl_top(rtlreg_t* dest) {
	...
	for(i=2; i>=0; i--) {
		*dest = paddr_read(cpu.esp + i*4, 4);
		Log("0x%08x: %08x", cpu.esp+i*4, *dest);
	}
}
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  ...
  // the seventh bit
	Log("update_ZF result is %x", *result);
	if(*result == 0)
		cpu.eflags.ZF = 1;
	else
		cpu.eflags.ZF = 0;
